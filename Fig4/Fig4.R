...
# Generate the three-layer patient timeline used for manuscript Figure 4A.
#
# Layer 1: total follow-up for every patient (required swimmer_data.csv)
# Layer 2: optional treatment/response intervals (swimmer_intervals.csv)
# Layer 3: surgery, recurrence, metastasis, and death events
#          (optional swimmer_events.csv; wide columns are used as fallback)

check_packages(c("dplyr", "ggplot2", "tidyr"))

patient_file <- file.path("data", "swimmer_data.csv")
interval_file <- file.path("data", "swimmer_intervals.csv")
event_file <- file.path("data", "swimmer_events.csv")
output_dir <- file.path("results", "swimmer_plot")
output_pdf <- file.path(output_dir, "Figure_4A_swimmer_plot.pdf")
ensure_directory(output_dir)

patients <- read_required_csv(
  patient_file,
  required_columns = c("patient_id", "arm", "CR", "followup_months")
)
assert_unique_id(patients)
patients$CR <- as_binary(patients$CR, "CR")
patients$followup_months <- as.numeric(patients$followup_months)

if (any(patients$followup_months < 0, na.rm = TRUE)) {
  stop("followup_months must be non-negative.")
}

if ("ongoing_followup" %in% names(patients)) {
  patients$ongoing_followup <- as_binary(patients$ongoing_followup, "ongoing_followup")
} else {
  patients$ongoing_followup <- 0L
}

patients$Response <- factor(patients$CR, levels = c(1, 0), labels = c("CR", "non-CR"))
patients <- patients[order(patients$followup_months, patients$arm), , drop = FALSE]
patients$patient_id <- factor(patients$patient_id, levels = patients$patient_id)

# Optional interval layer.
intervals <- NULL
if (file.exists(interval_file)) {
  intervals <- read_required_csv(
    interval_file,
    required_columns = c("patient_id", "start_month", "end_month", "interval_type")
  )
  intervals$start_month <- as.numeric(intervals$start_month)
  intervals$end_month <- as.numeric(intervals$end_month)
  intervals <- intervals[
    stats::complete.cases(intervals[, c("patient_id", "start_month", "end_month", "interval_type")]),
    ,
    drop = FALSE
  ]
  if (any(intervals$end_month < intervals$start_month)) {
    stop("Each swimmer interval must have end_month >= start_month.")
  }
  unknown_interval_ids <- setdiff(unique(intervals$patient_id), as.character(patients$patient_id))
  if (length(unknown_interval_ids) > 0L) {
    stop("Unknown patient_id in swimmer_intervals.csv: ", paste(unknown_interval_ids, collapse = ", "))
  }
  intervals$patient_id <- factor(intervals$patient_id, levels = levels(patients$patient_id))
}

# Optional event layer. Wide event columns are used when the long table is absent.
if (file.exists(event_file)) {
  events <- read_required_csv(
    event_file,
    required_columns = c("patient_id", "event_month", "event_type")
  )
  names(events)[names(events) == "event_month"] <- "month"
  names(events)[names(events) == "event_type"] <- "event"
  events$month <- as.numeric(events$month)
  events <- events[stats::complete.cases(events[, c("patient_id", "month", "event")]), , drop = FALSE]
} else {
  wide_event_columns <- intersect(
    c("surgery_months", "local_recurrence_months", "distant_metastasis_months", "death_months"),
    names(patients)
  )
  if (length(wide_event_columns) == 0L) {
    events <- data.frame(patient_id = character(), event = character(), month = numeric())
  } else {
    patients[wide_event_columns] <- lapply(patients[wide_event_columns], as.numeric)
    events <- tidyr::pivot_longer(
      patients,
      cols = dplyr::all_of(wide_event_columns),
      names_to = "event",
      values_to = "month"
    )
    events <- dplyr::filter(events, !is.na(.data$month))
    event_labels <- c(
      surgery_months = "Surgery",
      local_recurrence_months = "Local recurrence",
      distant_metastasis_months = "Distant metastasis",
      death_months = "Death"
    )
    events$event <- unname(event_labels[events$event])
  }
}
unknown_event_ids <- setdiff(unique(events$patient_id), as.character(patients$patient_id))
if (length(unknown_event_ids) > 0L) {
  stop("Unknown patient_id in swimmer event data: ", paste(unknown_event_ids, collapse = ", "))
}
events$patient_id <- factor(events$patient_id, levels = levels(patients$patient_id))
events$event <- factor(events$event, levels = unique(events$event))

plot <- ggplot2::ggplot(patients) +
  ggplot2::geom_segment(
    ggplot2::aes(
      x = 0,
      xend = .data$followup_months,
      y = .data$patient_id,
      yend = .data$patient_id,
...
if (nrow(events) > 0L) {
  shape_values <- stats::setNames(
    rep(c(16, 17, 15, 4, 8, 18), length.out = nlevels(events$event)),
    levels(events$event)
  )
  plot <- plot +
    ggplot2::geom_point(
      data = events,
      ggplot2::aes(x = .data$month, y = .data$patient_id, shape = .data$event),
      inherit.aes = FALSE,
      size = 2.5,
      colour = "black",
      fill = "white"
...
ongoing <- patients[patients$ongoing_followup == 1L, , drop = FALSE]
if (nrow(ongoing) > 0L) {
  arrow_length <- max(patients$followup_months, na.rm = TRUE) * 0.015
  plot <- plot + ggplot2::geom_segment(
    data = ongoing,
    ggplot2::aes(
      x = pmax(0, .data$followup_months - arrow_length),
      xend = .data$followup_months,
      y = .data$patient_id,
      yend = .data$patient_id
    ),
...
ggplot2::ggsave(
  ...
  plot = plot,
  width = 10,
  height = max(7, 0.16 * nrow(patients) + 2)
)

message("Swimmer plot saved to: ", output_pdf)
